...
procstate()
{
  ...
  // loop over process table looking for process id
  acquire(&ptable.lock);
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == SLEEPING){
       cprintf("%s \t Sleeping \t %d \t %d KBytes\n",p->name,p->pid,(p->sz%1000));
    }
    else if(p->state == RUNNING){
       cprintf("%s \t Running \t %d  \t %d KBytes\n",p->name,p->pid,(p->sz%1000));
    }
    else if(p->state == RUNNABLE){
       cprintf("%s \t Runnable \t %d \t %d KBytes\n",p->name,p->pid,(p->sz%1000));
    } 
    else if(p->state == ZOMBIE){
       cprintf("%s \t Zombie \t %d \t %d KBytes\n",p->name,p->pid,(p->sz%1000));
    }
    else if(p->state == EMBRYO){
       cprintf("%s \t Embryo \t %d \t %d KBytes\n",p->name,p->pid,(p->sz%1000));
    }
  }
  cprintf("-------------------------------------------\n");
  release(&ptable.lock);
